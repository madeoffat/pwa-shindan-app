<template>
  <div ref="wrapper" class="fd__wrapper">
    ...
    <v-text-field ref="textBox" type="text" class="fd__display-textbox" :disabled="disabled"
      :value="displayText" :placeholder="placeholder" readonly="readonly" @click="textBoxClicked"></v-text-field>
    <div class="fd__list" v-show="showList">
      <div class="fd__filter-input">
        <v-text-field ref="filterTextBox" type="text" class="fd__filter-textbox" placeholder="検索..." v-model="filterString"></v-text-field>
      </div>
      <ul>
        ...
        <li v-if="filteredItems.length === 0">候補がありません</li>
      </ul>
    </div>
  </div>
</template>
